 ...
 * @returns {Promise<Array<string>>} Uma promessa que resolve para um array de nomes de países (strings).
 */
export async function loadCountries() {
    ...
    try {
        ...
        const data = await response.json();
        // CORRIGIDO: Mapear para um array de strings de nomes de países
        if (Array.isArray(data)) {
            return data.map(countryObj => countryObj.nome).filter(name => typeof name === 'string');
        }
        return []; // Retorna um array vazio em caso de erro ou se 'data' não for um array
    } catch (error) {
