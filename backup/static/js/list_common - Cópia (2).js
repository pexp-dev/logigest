...
// Inclui: debounce, delegação de eventos, preservação de seleção,
// redimensionamento de colunas com persistência em localStorage,
// e dropdowns por linha que não ficam cortados.
...
export class ListManager {
    ...
    bindEvents() {
        ...
        // Dropdowns das linhas – versão robusta (corrigida para .dropdown-content)
        if (this.tbody) {
            this.tbody.addEventListener('click', (e) => {
                ...
                const menu = cell.querySelector('.dropdown-content');   // <-- CORREÇÃO AQUI

                if (!menu) return;

                // Fecha outros menus abertos
                this.closeAllRowDropdowns();
        ...
        document.addEventListener('click', (e) => {
            // Filtros, colunas, ações em massa
            ...
            // Fechar dropdowns das linhas se clicar fora do botão ou do menu
    ...
    closeAllRowDropdowns() {
        ...
        this.tbody.querySelectorAll('.dropdown-content').forEach(menu => {
            menu.classList.add('hidden');
            menu.closest('tr')?.classList.remove('dropdown-open');
        });
    }
    ...
    initColumnResize() {
        ...
        headers.forEach(th => {
            ...
            const onMouseUp = (e) => {
                if (isResizing) {
                    e.stopPropagation();
                    // Guardar largura no localStorage
                    const colIndex = Array.from(this.table.querySelectorAll('th')).indexOf(th);
                    if (colIndex >= 0) {
                        this.columnWidths[colIndex] = th.style.width;
