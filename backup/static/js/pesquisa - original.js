// pesquisa.js - Versão optimizada com persistência de estado e melhorias de performance
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    let colunasVisiveis = ['codigo', 'name', 'descricao', 'acoes'];
    let ordenarPor = null;
    let ordemAscendente = true;

    // Carregar colunas guardadas
    ...
    // Mapeamento entre texto do cabeçalho e nome do campo no objeto artigo
    ...
    async function restoreAndRefresh() {
        ...
        // Restaurar valores nos inputs e selects
        ...
        await carregarTodosArtigos(); // recarrega dados frescos
        aplicarFiltros(); // aplica os filtros restaurados
        renderizarTabela();
        return true;
    }
    ...
    async function carregarTodosArtigos() {
        ...
        try {
            ...
            // Atualizar checkboxes das colunas
    ...
    function aplicarFiltros() {
        ...
        const filtros = {
            ...
            modelo_catalogo: document.getElementById('filtro-modelo-catalogo').value,
            estado: document.getElementById('filtro-estado').value,
            seller_product_name: document.getElementById('filtro-seller-product-name').value.toLowerCase().trim(),
            obs: document.getElementById('filtro-obs').value.toLowerCase().trim(),
            modelo: document.getElementById('filtro-modelo').value.toLowerCase().trim(),
            tipo_cartao: document.getElementById('filtro-tipo-cartao').value,
            dimensao_x: document.getElementById('filtro-dimensao-x').value,
            dimensao_y: document.getElementById('filtro-dimensao-y').value,
            dimensao_z: document.getElementById('filtro-dimensao-z').value,
            ultima_fatura: document.getElementById('filtro-ultima-fatura').value.toLowerCase().trim()
        };

        artigosFiltrados = todosArtigos.filter(art => {
            // Pesquisa por texto
            if (termo) {
                const match = (art.codigo && art.codigo.toLowerCase().includes(termo)) ||
                    (art.name && art.name.toLowerCase().includes(termo)) ||
                    (art.seller_product_name && art.seller_product_name.toLowerCase().includes(termo)) ||
                    (art.obs && art.obs.toLowerCase().includes(termo)) ||
                    (art.oficial?.descricao && art.oficial.descricao.toLowerCase().includes(termo));
                if (!match) return false;
            }

            // Filtros exatos ou parciais
            ...
            if (filtros.seller_product_name && !(art.seller_product_name && art.seller_product_name.toLowerCase().includes(filtros.seller_product_name))) return false;
            if (filtros.obs && !(art.obs && art.obs.toLowerCase().includes(filtros.obs))) return false;
            if (filtros.modelo && !(art.oficial?.modelo && art.oficial.modelo.toLowerCase().includes(filtros.modelo))) return false;
            if (filtros.tipo_cartao && art.oficial?.tipo_cartao !== filtros.tipo_cartao) return false;
            if (filtros.dimensao_x && art.oficial?.dimensoes?.x != filtros.dimensao_x) return false;
            if (filtros.dimensao_y && art.oficial?.dimensoes?.y != filtros.dimensao_y) return false;
            if (filtros.dimensao_z && art.oficial?.dimensoes?.z != filtros.dimensao_z) return false;
            if (filtros.ultima_fatura && !(art.ultima_fatura && art.ultima_fatura.toLowerCase().includes(filtros.ultima_fatura))) return false;
    ...
    function renderizarTabela() {
        ...
        for (const art of artigosPagina) {
            ...
            for (const col of colunasVisiveis) {
                ...
                if (col === 'acoes') {
                    ...
                    link.href = `/validar/artigo/${codigo}`;
        ...
        // Esconder colunas conforme visíveis
    ...
    async function init() {
        ...
        // Persistir estado ao sair da página (opcional)
        window.addEventListener('beforeunload', () => saveSearchState());
        // Recuperar estado quando a página é restaurada do bfcache
        window.addEventListener('pageshow', (event) => {
            if (event.persisted) restoreAndRefresh();
        });
    }
